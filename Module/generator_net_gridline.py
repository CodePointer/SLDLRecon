...
class GeneratorNet(nn.Module):
    ...
    def __init__(self, root_path, batch_size, opts=None):
        ...
        self.h_out_layer = nn.Sequential(
            nn.Linear(1024, self.Lh),
            nn.Tanh()
        )
        self.w_out_layer = nn.Sequential(
            nn.Linear(1024, self.Lw),
            nn.Tanh()
        )
    ...
    def forward(self, x):
        ...
        h_vec_out = self.h_out_layer(fc_res).transpose(1, 2)  # [N, Lh, 1]
        w_vec_out = self.w_out_layer(fc_res).transpose(1, 2)  # [N, Lw, 1]

        scale = 1.3
        h_vec_out = (h_vec_out * scale / 2 + 0.5) * self.H
        w_vec_out = (w_vec_out * scale / 2 + 0.5) * self.W

        h_dis = torch.pow(self.h_mat - h_vec_out, 2) * self.range_squeeze  # [N, Lh, H]
        w_dis = torch.pow(self.w_mat - w_vec_out, 2) * self.range_squeeze  # [N, Lw, W]
